...
const Login = () => {
  ...
  const setUsernameMet = (event) => {
    ...
    if (/^\S+@\S+\.\S+$/.test(event.target.value) || event.target.value.length === 0) {
  ...
  return (
    <div className="login-form">
      ...
      <form onSubmit={loginSubmit}>
        ...
        {!inputState.validEmail ? <div>Enter Valid Email</div> : null}
        <br />
        ...
        />
        {!inputState.validPassword ? <div>Enter Valid Password</div> : null}
        <br />
